...
if ( ! function_exists( 'rex' ) ) :
    ...
    function rex_setup() {
        ...
        register_nav_menus( array(
            'primary' => esc_html__( 'PrimaryNav', 'rex' ),
            'footer' => esc_html__( 'FooterNav', 'rex' ),
        ) );
...
function sendMail(){
    if ( wp_verify_nonce( $_POST['nonce'], 'form-send-mail' ) ) {
        ...
        $emailrecive = 'filingame12@gmail.com';
        $headers = 'From: '.$name.'<'.$email.'>';

        if( wp_mail( $emailrecive, 'Order from '.$name, $message, $headers) ) {
            echo 'Mail sent!';
        } else {
            echo 'Error!';
        }
